...
class StraightController:
    def __init__(self):
        ...
        self.thrust_value = 1600  # 직진용 PWM 값
    ...
    def run(self):
        ...
        while not rospy.is_shutdown():
            msg = Control()
            msg.thruster_p = UInt16(self.thrust_value)
            msg.thruster_s = UInt16(self.thrust_value)

            self.control_pub.publish(msg)
            rospy.loginfo(f"Published thrust (p/s): {self.thrust_value}, {self.thrust_value}")
