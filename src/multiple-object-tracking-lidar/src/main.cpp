...
std::pair<int, int> findIndexOfMin(std::vector<std::vector<float>> distMat) {
  ...
  for (int i = 0; i < (int)distMat.size(); i++)
    for (int j = 0; j < (int)distMat.at(0).size(); j++) {
...
void KFT(const std_msgs::Float32MultiArray ccs) {
  ...
  // greedy assignment (global min row/col masking)
  objID.clear();
  objID.resize(6);
  ...
  for (int clusterCount = 0; clusterCount < 6; clusterCount++) {
    ...
    for (int row = 0; row < (int)distMat.size(); row++)
      distMat[row][minIndex.second] = 10000.0f;
  }

  // markers (KF predictions)
  ...
  // publish association result
  std_msgs::Int32MultiArray obj_id;
  for (auto &id : objID) obj_id.data.push_back(id);
  ...
  float meas0[2] = {cc[0][0], cc[0][1]};
  float meas1[2] = {cc[1][0], cc[1][1]};
  float meas2[2] = {cc[2][0], cc[2][1]};
  float meas3[2] = {cc[3][0], cc[3][1]};
  float meas4[2] = {cc[4][0], cc[4][1]};
  float meas5[2] = {cc[5][0], cc[5][1]};
  ...
  if (!(meas0Mat.at<float>(0,0) == 0.0f || meas0Mat.at<float>(1,0) == 0.0f)) KF0.correct(meas0Mat);
...
void cloud_cb(const sensor_msgs::PointCloud2ConstPtr &input)
{
  if (firstFrame) {
    // init KFs
    float dvx = 0.01f, dvy = 0.01f, dx = 1.0f, dy = 1.0f;
    ...
    float sigmaP = 0.01f, sigmaQ = 0.1f;
    ...
    for (auto it = cluster_indices.begin(); it != cluster_indices.end(); ++it) {
      ...
      float x = 0.0f, y = 0.0f; int numPts = 0;
      ...
      pcl::PointXYZ centroid; centroid.x = x / std::max(1,numPts); centroid.y = y / std::max(1,numPts); centroid.z = 0.0f;
    ...
    while (clusterCentroids.size() < 6) {
      pcl::PointXYZ centroid; centroid.x = 0.0f; centroid.y = 0.0f; centroid.z = 0.0f;
      clusterCentroids.push_back(centroid);
    }

    // set initial states
    KF0.statePre.at<float>(0) = clusterCentroids[0].x; KF0.statePre.at<float>(1) = clusterCentroids[0].y; KF0.statePre.at<float>(2) = 0; KF0.statePre.at<float>(3) = 0;
    KF1.statePre.at<float>(0) = clusterCentroids[1].x; KF1.statePre.at<float>(1) = clusterCentroids[1].y; KF1.statePre.at<float>(2) = 0; KF1.statePre.at<float>(3) = 0;
    KF2.statePre.at<float>(0) = clusterCentroids[2].x; KF2.statePre.at<float>(1) = clusterCentroids[2].y; KF2.statePre.at<float>(2) = 0; KF2.statePre.at<float>(3) = 0;
    KF3.statePre.at<float>(0) = clusterCentroids[3].x; KF3.statePre.at<float>(1) = clusterCentroids[3].y; KF3.statePre.at<float>(2) = 0; KF3.statePre.at<float>(3) = 0;
    KF4.statePre.at<float>(0) = clusterCentroids[4].x; KF4.statePre.at<float>(1) = clusterCentroids[4].y; KF4.statePre.at<float>(2) = 0; KF4.statePre.at<float>(3) = 0;
    KF5.statePre.at<float>(0) = clusterCentroids[5].x; KF5.statePre.at<float>(1) = clusterCentroids[5].y; KF5.statePre.at<float>(2) = 0; KF5.statePre.at<float>(3) = 0;

    firstFrame = false;
    for (int i = 0; i < 6; i++) {
      geometry_msgs::Point pt; pt.x = clusterCentroids[i].x; pt.y = clusterCentroids[i].y; pt.z = 0.0;
      prevClusterCenters.push_back(pt);
    }
  }
  else {
    // cluster & associate each frame
    pcl::PointCloud<pcl::PointXYZ>::Ptr input_cloud(new pcl::PointCloud<pcl::PointXYZ>);
    pcl::search::KdTree<pcl::PointXYZ>::Ptr tree(new pcl::search::KdTree<pcl::PointXYZ>);
    pcl::fromROSMsg(*input, *input_cloud);
    tree->setInputCloud(input_cloud);
    ...
    for (int i = 0; i < 6; i++) {
      cc.data.push_back(clusterCentroids[i].x);
      cc.data.push_back(clusterCentroids[i].y);
      cc.data.push_back(clusterCentroids[i].z);
    }
    ...
    for (auto it = objID.begin(); it != objID.end(); it++) {
      switch (i) {
      case 0: publish_cloud(pub_cluster0, cluster_vec[*it]); publishedCluster[i]=true; i++; break;
      case 1: publish_cloud(pub_cluster1, cluster_vec[*it]); publishedCluster[i]=true; i++; break;
      case 2: publish_cloud(pub_cluster2, cluster_vec[*it]); publishedCluster[i]=true; i++; break;
      case 3: publish_cloud(pub_cluster3, cluster_vec[*it]); publishedCluster[i]=true; i++; break;
      case 4: publish_cloud(pub_cluster4, cluster_vec[*it]); publishedCluster[i]=true; i++; break;
      case 5: publish_cloud(pub_cluster5, cluster_vec[*it]); publishedCluster[i]=true; i++; break;
      default: break;
      }
    }
  }
